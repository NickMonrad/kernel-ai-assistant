...
def augment(wav: np.ndarray, sr: int, rng: random.Random) -> np.ndarray:
    ...
    tensor = tensor / peak * rng.uniform(0.7, 0.95)

    return tensor.squeeze(0).detach().numpy()


def augment_real(wav_path: Path, sr_expected: int, n: int, rng: random.Random) -> list[np.ndarray]:
    ...
    return [augment(wav, sr_expected, rng) for _ in range(n)]
...
def main():
            ...
            for phrase in PHRASES:
                try:
                    ...
                    for _ in range(args.augment_factor):
                        wav_aug = augment(wav16, 16000, rng)
    ...
    if args.real_dir:
        ...
        for wav_path in real_wavs:
            ...
            for aug_wav in augment_real(wav_path, 16000, args.augment_factor, rng):
