...
# ---------------------------------------------------------------------------
# PR comment builder (used by --post-pr)
# ---------------------------------------------------------------------------
...
def _build_pr_comment(data: dict, report_path: Path) -> str:
    """Build a GitHub-flavoured markdown comment from a loaded JSON report dict."""
    summary = data.get("summary", {})
    total   = summary.get("total", 0)
    passed  = summary.get("passed", 0)
    xfails  = summary.get("xfail", 0)
    failed  = summary.get("failed", 0)
    ...
    failures = [
        ...
        if not r.get("xfail") and not (r.get("intent_passed") and r.get("params_passed", True))
    ]
    if failures:
        lines += [
            "",
        ...
        for r in failures[:10]:
            actual = r.get("actual_intent") or "NO_MATCH"
            lines.append(
                f"| {r.get('index', '')} | {r.get('message', '')} "
                f"| `{r.get('expect_intent', '')}` | `{actual}` |"
            )
        if len(failures) > 10:
            lines.append(f"| … | *and {len(failures) - 10} more* | | |")

    # Phase breakdown — only when results carry phase info
    results_with_phase = [r for r in data.get("results", []) if r.get("phase")]
    if results_with_phase:
        from collections import defaultdict
        phase_data: dict[str, dict[str, int]] = defaultdict(lambda: {"pass": 0, "fail": 0, "xfail": 0})
        for r in data.get("results", []):
            key = r.get("phase") or "unknown"
            if r.get("xfail") and not r.get("intent_passed"):
                phase_data[key]["xfail"] += 1
            elif r.get("intent_passed") and r.get("params_passed", True):
        ...
        for phase_name in seen:
            ...
            lines.append(f"| {phase_name} | {d['pass']} | {d['fail']} | {d['xfail']} |")
        lines += ["", "</details>"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--post-pr",
        action="store_true",
        help="Post a markdown test summary comment to the open PR for the current branch",
    )
    ...
    if args.post_pr:
        body = _build_pr_comment(data, input_path)
