...
PHASES: list[tuple[str, list[TestCase]]] = [
    ...
    ("lists", [
        ...
        # §7E: Complementary positive test — must route to add_to_list, not save_memory
    ...
    ("false_positives", [
        # ── §2B: Date/Time false-positives — must not trigger get_time ──────
        TestCase("What year is this movie set in",
                 forbidden_intents=["get_time"],
                 expect_llm_fallthrough=True),
        TestCase("What month is this charge for",
                 forbidden_intents=["get_time"],
                 expect_llm_fallthrough=True),
        TestCase("What is the week's schedule",
                 forbidden_intents=["get_time"],
                 expect_llm_fallthrough=True),
        TestCase("Is it Friday's episode tonight",
                 forbidden_intents=["get_time"],
                 expect_llm_fallthrough=True),
        TestCase("What time should I leave",
                 forbidden_intents=["get_time"],
                 expect_llm_fallthrough=True),
        TestCase("What's the best time of year to visit Japan",
                 forbidden_intents=["get_time"],
                 expect_llm_fallthrough=True),
        # ── §3B: Calendar false-positive — must not trigger create_calendar_event ──
        TestCase("Send a calendar invite to John",
                 forbidden_intents=["create_calendar_event"],
                 expect_llm_fallthrough=True),
        # ── §3C: Alarm/Timer disambiguation ────────────────────────────────
        TestCase("Set a 5 minute egg timer",
                 forbidden_intents=["set_alarm"],
                 allowed_intents=["set_timer"],
                 expect_llm_fallthrough=True),
        # ── §5E: List false-positives ──────────────────────────────────────
        TestCase("List all the capitals of Europe",
                 forbidden_intents=["create_list", "get_list_items"],
                 expect_llm_fallthrough=True),
        TestCase("Create a plan for my week",
                 forbidden_intents=["create_list"],
                 expect_llm_fallthrough=True),
        TestCase("Add some detail to your explanation",
                 forbidden_intents=["add_to_list"],
                 expect_llm_fallthrough=True),
        # ── §7E: Memory false-positives ────────────────────────────────────
        TestCase("I remember when we talked about this",
                 forbidden_intents=["save_memory"],
                 expect_llm_fallthrough=True),
        TestCase("Don't forget to add milk",
                 forbidden_intents=["save_memory"],
                 expect_llm_fallthrough=True),
        # ── §8C: Weather false-positives ──────────────────────────────────
        TestCase("How hot was the summer of '69",
                 forbidden_intents=["get_weather"],
                 expect_llm_fallthrough=True),
        TestCase("Is it going to be a long winter",
                 forbidden_intents=["get_weather"],
                 expect_llm_fallthrough=True),
