...
For acoustic wake-word runs, use `scripts/acoustic_wake_reliability_runner.py` rather than hand-authoring evidence. Its `evidence.json` already conforms to the normalised schema extension. Publish the complete run directory so each case's relative `artifact_refs` remains resolvable. Diagnostic, smoke, interrupted, and feasibility runs are visible on the dashboard but do not satisfy the release gate.
