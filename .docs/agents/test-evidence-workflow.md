...
7. Triggers the "Publish test dashboard" workflow with `gh` if authentication is available.
...
7. **Trigger dashboard** — if `gh` authentication is available, trigger the "Publish test dashboard" workflow to refresh the dashboard.
