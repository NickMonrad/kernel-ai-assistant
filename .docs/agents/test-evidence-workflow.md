...
For **on-device evidence**, the reviewer or user:

1. Collects the ADB harness report (JSON).
2. Normalises it to match the evidence schema.
3. Runs `scripts/publish_test_evidence.py` locally with the appropriate arguments.
4. Manually triggers the "Publish test dashboard" workflow — local publishing does not auto-refresh the dashboard.
...
- The report must be normalised to match the evidence schema (`docs/testing/test-evidence-schema.md`).
- Publishing is done locally via `scripts/publish_test_evidence.py` — the "Publish test evidence" GitHub Actions workflow only supports `source=ci`.
- After publishing locally, manually trigger the "Publish test dashboard" workflow to refresh the dashboard.
- Future automation may add local dispatch capabilities, but currently the dashboard does not auto-refresh for locally-published evidence.
