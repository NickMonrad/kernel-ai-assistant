...
| Arctic Embed M v1.5 + WordPiece vocabulary | Required embedding/RAG model and tokenizer. | `Snowflake/snowflake-arctic-embed-m-v1.5` at pinned revision `e58a8f756156a1293d763f17e3aae643474e9b8a`, Apache-2.0. | Both SHA-256-pinned assets are published in a public, versioned GitHub prerelease; anonymous downloads matched the pins. GitHub reports `immutable=false`, and the current account lacks repository admin permission to enable immutable-release protection. Hold release sign-off; do not describe the distribution as immutable. |
