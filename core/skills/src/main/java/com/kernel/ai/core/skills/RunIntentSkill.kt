...
class RunIntentSkill @Inject constructor(
    ...
    override val fullInstructions: String = buildString {
        ...
        appendLine("  convert_cooking_measure — params: amount, from_unit, ingredient, to_unit (ingredient-aware volume↔mass cooking conversion; defaults to grams when the user asks what something weighs)")
        ...
        appendLine()
        appendLine("Cooking-conversion rule: for ingredient-aware kitchen volume↔mass questions, ALWAYS call runIntent with intentName=convert_cooking_measure.")
        appendLine("Pass amount as a decimal string, pass kitchen units in from_unit/to_unit, and include ingredient exactly as the user said it.")
        appendLine("Use this for cases like 3 tbsp butter → grams or 400 g flour → cups. If the question is still plain same-category unit math, keep using convert_units.")
        appendLine("Do NOT invent cooking densities. The native path uses a small built-in ingredient table and will fail clearly if the ingredient is unsupported.")
        appendLine()
        ...
        appendLine("Do NOT invent, estimate, or claim real-time exchange rates in model text. The native path will return the latest available ECB-backed rate date and fail closed if rates are unavailable or too stale.")
        appendLine()
