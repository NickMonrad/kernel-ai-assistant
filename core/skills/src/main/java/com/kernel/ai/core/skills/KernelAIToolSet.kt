...
class KernelAIToolSet @Inject constructor(
    ...
    @Tool(description = "Execute a native Android device action: flashlight, alarms, timers, calendar events, email, SMS, phone calls, Do Not Disturb, volume, Wi-Fi, Bluetooth, airplane mode, hotspot, media playback (local/YouTube/Spotify/Netflix/Plex), navigation, app launching, battery status, current time, current date, and list management (shopping lists, grocery lists, to-do lists). NOT for weather, web search, memory recall, or general knowledge questions.")
    fun runIntent(
        @ToolParam(description = "The intent action: toggle_flashlight_on, toggle_flashlight_off, send_email, send_sms, make_call, set_alarm, set_timer, create_calendar_event, toggle_dnd_on, toggle_dnd_off, toggle_wifi, toggle_bluetooth, toggle_airplane_mode, toggle_hotspot, set_volume, play_media, play_media_album, play_media_playlist, play_youtube, play_spotify, play_plexamp, play_youtube_music, play_netflix, play_plex, navigate_to, find_nearby, open_app, get_battery, get_time, get_date, add_to_list, bulk_add_to_list, create_list, get_list_items, remove_from_list") intentName: String,
        @ToolParam(description = "Additional parameters as key:value pairs in JSON. For set_alarm: {\"time\":\"10pm\"} or {\"time\":\"7:30am\",\"day\":\"monday\",\"label\":\"Wake up\"}. For set_timer: {\"duration_seconds\":\"180\"}. For send_email: {\"subject\":\"Hi\",\"body\":\"Text\"}. For send_sms: {\"contact\":\"Mom\",\"message\":\"Text\"}. For make_call: {\"contact\":\"Dad\"}. For create_calendar_event: {\"title\":\"Meeting\",\"date\":\"2026-04-15\",\"time\":\"12:30\"}. For set_volume: {\"value\":\"50\",\"is_percent\":\"true\"}. For play_media: {\"query\":\"Song Name\",\"artist\":\"Artist\"}. For play_plex: {\"title\":\"Movie Name\"}. For navigate_to: {\"destination\":\"airport\"}. For toggle_wifi/bluetooth/airplane_mode/hotspot: {\"state\":\"on\"}. For open_app: {\"app_name\":\"Spotify\"}. For toggle_dnd/flashlight/get_battery/get_time/get_date: {}. For add_to_list: {\"item\":\"milk\",\"list_name\":\"shopping list\"}. For bulk_add_to_list: {\"items\":\"milk,eggs,bread\",\"list_name\":\"shopping list\"}. For create_list: {\"list_name\":\"meal plan\"}. For get_list_items: {\"list_name\":\"shopping list\"}. For remove_from_list: {\"item\":\"milk\",\"list_name\":\"shopping list\"}") parameters: String,
    ): Map<String, String> {
        toolCalledInThisTurn = true
