...
class WakeWordGateExitSummaryTest {
    ...
    private fun installRecorder(): MutableList<AcousticEvent> {
    ...
    /** Completes the 16-frame embedding context ring so the max-confidence
     *  window energy is computable (values 100..1600 step 100). */
    private fun fillEmbeddingRing(diag: WakeWordGateExitDiagnostics) {
        for (value in 1..16) diag.onEmbeddingFrameRms(value * 100f)
    }

    // ── Journal emission (#1410 evidence retention) ────────────────────────

    @Test
    fun `gate re-entry journals the episode summary with exact energy fields`() {
        val events = installRecorder()
        try {
            val diag = newDiag(generationId = 7)
            diag.onGateEntered(100)
            diag.onGateExited(200)
            diag.onEpisodeFrameRms(3593.4f)
            fillEmbeddingRing(diag)
            diag.onStage3Evaluation(0.6335f, 210)

            diag.onGateEntered(400)

            val recorded = events.single()
            assertEquals(AcousticEventType.GATE_EPISODE_SUMMARY, recorded.type)
            assertEquals(7L, recorded.generationId)
            assertEquals(
                mapOf(
                    "stage3_evals" to "1",
                    "max_confidence" to "0.6335",
                    "max_confidence_offset_frames" to "10",
                    "low_verify_entered" to "false",
                    "low_verify_accepted" to "false",
                    "gated_probe_executions" to "0",
                    "episode_peak_rms" to "3593.4",
                    "max_window_peak_rms" to "1600.0",
                    "max_window_mean_rms" to "850.0",
                ),
                recorded.metadata,
            )
        } finally {
    ...
    @Test
    fun `finish journals the final open episode summary`() {
        val events = installRecorder()
        try {
            val diag = newDiag()
            diag.onGateEntered(100)
            diag.onGateExited(200)
            fillEmbeddingRing(diag)
            diag.onStage3Evaluation(0.911f, 212)

            assertTrue(diag.finish() != null)

            val recorded = events.single()
            assertEquals(AcousticEventType.GATE_EPISODE_SUMMARY, recorded.type)
            assertEquals("0.911", recorded.metadata["max_confidence"])
        } finally {
            resetRecorder()
        }
    }
    ...
    @Test
    fun `incomplete window energy is journalled as none not fabricated`() {
        val events = installRecorder()
        try {
            val diag = newDiag()
            diag.onGateEntered(100)
            diag.onGateExited(200)
            diag.onEpisodeFrameRms(7438.4f)
            diag.onStage3Evaluation(0.5f, 202) // embedding ring incomplete

            diag.onGateEntered(400)

            val recorded = events.single()
            assertEquals("7438.4", recorded.metadata["episode_peak_rms"])
            assertEquals("none", recorded.metadata["max_window_peak_rms"])
            assertEquals("none", recorded.metadata["max_window_mean_rms"])
        } finally {
            resetRecorder()
        }
    }
    ...
    @Test
    fun `no episode summary means no journal event`() {
        val events = installRecorder()
        try {
            val diag = newDiag()
            diag.onGateEntered(100) // no intervening exit
            diag.onGateEntered(400)

            assertTrue(events.isEmpty())

            // finish() with no open episode emits nothing.
            val diag2 = newDiag()
            diag2.onGateEntered(100)
            diag2.onGateExited(200)
            diag2.onGateEntered(300) // closes the episode -> one summary
