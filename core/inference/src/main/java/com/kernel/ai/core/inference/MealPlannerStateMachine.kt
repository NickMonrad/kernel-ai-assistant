...
object MealPlannerStateMachine {
    ...
    enum class State(val label: String) {
        ...
        PLAN_DRAFT_READY("High-level plan ready"),
        PLAN_CONFIRMED("Plan confirmed"),
        GENERATING_RECIPE("Generating recipes"),
        RECIPE_REVIEW("Reviewing recipes"),
    ...
    fun validTransitions(current: State): List<State> = when (current) {
        ...
        State.PLAN_DRAFT_READY -> listOf(State.PLAN_CONFIRMED, State.CANCELLED)

        State.PLAN_CONFIRMED -> listOf(State.GENERATING_RECIPE, State.CANCELLED)
    ...
    fun stateSkillName(state: State): String? = when (state) {
        ...
        State.PLAN_CONFIRMED, State.GENERATING_RECIPE, State.RECIPE_REVIEW,
